...
const ResponsiveModalVariants = cva(
  cn(
    "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 overflow-y-auto bg-slate-900 border-slate-600/40",
...
>(({ side = "bottom", className, children, ...props }, ref) => (
  <ResponsiveModalPortal>
    ...
    >
      {children}
      <ResponsiveModalClose className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-slate-950 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-slate-300 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-slate-800 data-[state=open]:text-slate-400">
...
const ResponsiveModalHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1.5 text-center sm:text-left mb-4 lg:mb-0",
      className,
    )}
    {...props}
  />
